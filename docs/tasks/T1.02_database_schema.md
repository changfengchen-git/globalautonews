...
## 任务目标
创建 PostgreSQL 数据库初始化脚本，包含所有核心表、索引和函数。
...
1. ✅ `db/init.sql` 文件已创建
2. ✅ 启动 PostgreSQL 容器后 SQL 可以成功执行：
   ```bash
   docker compose up -d postgres
   docker compose exec postgres psql -U gan -d globalautonews -c "\dt"
   ```
   应该看到 5 张表：sources, articles, event_clusters, source_candidates, crawl_logs
3. ✅ 所有索引已创建：
   ```bash
   docker compose exec postgres psql -U gan -d globalautonews -c "\di"
   ```
4. ✅ pgvector 扩展已启用：
   ```bash
   docker compose exec postgres psql -U gan -d globalautonews -c "SELECT * FROM pg_extension WHERE extname='vector';"
   ```
5. ✅ 触发器工作正常：更新 sources 表记录后 updated_at 自动更新
